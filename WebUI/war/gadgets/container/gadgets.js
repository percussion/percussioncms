...
gadgets.IfrGadgetService.prototype.setTitle = function(title) {
  ...
  for(i = 0; i < gadgets.data.setTitleListeners.length; i++)
  {
     gadgets.data.setTitleListeners[i](this.f + '_title');
  }
};
...
gadgets.IfrGadgetService.prototype.setUserPref = function(editToken, name, value) {
  ...
  for (var i = 2, j = arguments.length; i < j; i += 2) { 
    gadget.userPrefs[arguments[i - 1]] = arguments[i]; 
  }
...
gadgets.StaticLayoutManager.prototype.addGadgetChromeId =
    function(key, theid) {
       if(typeof(this.gadgetChromeIds_) != 'object')
          this.gadgetChromeIds_ = new Array();
  this.gadgetChromeIds_[key] = theid;
};
...
gadgets.Gadget.prototype.getUserPrefValue = function(name) {
  ...
  return typeof(pref) != 'undefined' && pref != null ? 
      pref : this.userPrefs['default']; 
}; 
...
gadgets.Gadget.prototype.render = function(chrome) {
  if (chrome) {
    ...
    this.getContent(function(content) {
      ...
      gadget.refresh();
      //window.frames[gadget.getIframeId()].location.href = gadget.getIframeUrl();
    });
  }
};
...
gadgets.IfrGadget.prototype.getTitleBarContent = function(continuation) {
  ...
  continuation('<div id="' + this.cssClassTitleBar + '-' + this.id +
      '" class="' + this.cssClassTitleBar + '"><div id="' +
...
gadgets.IfrGadget.prototype.getIframeUrl = function() {
  //Get pssessionid and add it to iframe url
  var pssessionid = jQuery.cookie("pssessionid");
  var d = new Date();
  return this.serverBase_ + 'cm/gadgets/ifr?' +
...
gadgets.IfrGadget.prototype.handleOpenUserPrefsDialog = function(callback) {
  if (this.userPrefsDialogContentLoaded) {
    ...
    script.src = '/cm/gadgets/settings/form?mid=' + this.id +
        '&output=js' + this.getUserPrefsParams() +  '&gurl=' + this.specUrl;
...
gadgets.IfrGadget.prototype.buildUserPrefsDialog = function(content) {
  ...
  userPrefsDialog.innerHTML = content +
      ...
      '">&nbsp;<button class="perc-save gadgets-gadget-small-btn btn btn-primary" onclick="gadgets.container.getGadget(' +
      this.id +').handleSaveUserPrefs()">'+I18N.message("perc.ui.common.label@Save")+'</button> &nbsp;<button class="perc-cancel gadgets-gadget-small-btn btn btn-primary" onclick="gadgets.container.getGadget(' +
      this.id +').handleCancelUserPrefs()">'+I18N.message("perc.ui.common.label@Cancel")+'</button>&nbsp;</div><div class="perc-gadget-titlebar-shadow"/>';
...
gadgets.IfrGadget.prototype.showUserPrefsDialog = function(opt_show) {
  var userPrefsDialog = document.getElementById(this.getUserPrefsDialogId());
  var isShow = (opt_show || opt_show === undefined);
  if(isShow)
     this.extractFormToUserPrefs();
  userPrefsDialog.style.display = isShow ? '' : 'none';
};
...
gadgets.IfrGadget.prototype.extractFormToUserPrefs = function(){
...
gadgets.IfrGadget.prototype.handleCancelUserPrefs = function() {
  this.hideUserPrefsDialog();
  this.restoreFormUserPrefs();
};

gadgets.IfrGadget.prototype.restoreFormUserPrefs = function() {
  var numFields = document.getElementById('m_' + this.id +
      '_numfields').value;
  for (var i = 0; i < numFields; i++) {
    var input = document.getElementById('m_' + this.id + '_' + i);
    var userPrefNamePrefix = 'm_' + this.id + '_up_';
    var userPrefName = input.name.substring(userPrefNamePrefix.length);
    var userPrefValue = this.userPrefs[userPrefName];
    if(input.type == 'checkbox')
    {
       input.checked = 
          typeof(userPrefValue) != "undefined" && userPrefValue == "true" ? true : false;
    }
    else
    {
       input.value = 
          typeof(userPrefValue) != "undefined" ? userPrefValue : "";
    }
...
gadgets.IfrGadget.prototype.refresh = function() {
  var iframeId = this.getIframeId();
  var frameUrl = this.getIframeUrl();
...
gadgets.Container.prototype.addGadget = function(gadget) {
  if(typeof(gadget.id) == 'undefined')
  {
     gadget.id = this.getNextGadgetInstanceId();     
  }
  this.addGadgetInstanceId(gadget.id);
  this.gadgets_[this.getGadgetKey_(gadget.id)] = gadget;
  gadget.userPrefs = this.userPrefStore.getPrefs(gadget); 
};
...
gadgets.Container.prototype.removeGadget = function(instanceId){
   var gadget = this.gadgets_[this.getGadgetKey_(instanceId)];
   gadget.userPrefs = {};
   gadgets.container.userPrefStore.removePrefs(gadget);
   delete this.gadgets_[this.getGadgetKey_(instanceId)];
   this.removeGadgetInstanceId(instanceId);
}
...
gadgets.Container.prototype.nextGadgetInstanceIds_ = [];

gadgets.Container.prototype.setGadgetInstanceIds = function(ids)
{
   this.clearGadgetInstanceIds();
   for(var i = 0; i < ids.length; i++)
      this.addGadgetInstanceId(ids[i]);
}

gadgets.Container.prototype.addGadgetInstanceId = function(id)
{
   this.nextGadgetInstanceIds_.push(id);
}

gadgets.Container.prototype.removeGadgetInstanceId = function(id)
{
   var idx = -1;
   for(var i = 0; i < this.nextGadgetInstanceIds_.length; i++)
   {
      if(this.nextGadgetInstanceIds_[i] == id)
      {
         idx = i;
         break;
      }
   ...
   if(idx != -1)
      this.nextGadgetInstanceIds_.splice(idx, 1);
}

gadgets.Container.prototype.clearGadgetInstanceIds = function()
{
   this.nextGadgetInstanceIds_ = [];
}
...
gadgets.Container.prototype.getNextGadgetInstanceId = function() {
  ...
  var temp = -1;
  var inc = true;
  this.nextGadgetInstanceIds_.sort( function (a,b) { return a-b });
  for(var i = 0; i < this.nextGadgetInstanceIds_.length; i++)
  {
     temp = this.nextGadgetInstanceIds_[i];
     if((temp - nextId) > 1)
     {
        ++nextId;
        inc = false;
...
gadgets.data.setTitleListeners = [];
gadgets.Container.prototype.addSetTitleListener = function(listener){
   gadgets.data.setTitleListeners.push(listener);   
}
