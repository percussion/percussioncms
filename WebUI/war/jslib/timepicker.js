...
$.datepicker._checkExternalClick = function (event) {
    ...
    if (($target.parents('#' + $.timepicker._mainDivId).length == 0)) {
		...
		($target.hasClass($.datepicker.markerClassName) && $.datepicker._curInst != inst)) 
			$('html').click(function (){
				$.datepicker._hideDatepicker();//Removing Click event to correct functionality
			})
    }
	return;
};
...
Timepicker.prototype = {
    ...
    _generateHtml: function ()
    {
        ...
        html += '<tr><td><div id="hourSlider" class="slider"></div></td><td><div id="minuteSlider" class="slider"></div></td></tr>';
        html += '</table>';
