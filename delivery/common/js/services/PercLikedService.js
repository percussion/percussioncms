...
(function($){
    ...
    varlikesServiceUrl = "/perc-comments-services/likes/";
     ...
     */
    varlikeThis = function(callback) {
		if("undefined" === typeof (callback) || null === callback) {return false};
		
		varlikePathname = likeId(window.location.pathname);
     ...
     * @param callback (function) function to call with response from server
     */
	varunlike = function(site, id, type, callback) {
        varlikeServiceUrl = likesServiceUrl + "unlike/" + site + "/" + type + "/" + id;
        $.PercServiceUtils.makeXdmJsonRequest(null, likeServiceUrl, $.PercServiceUtils.TYPE_POST, function(status, results)
        {
            if(status === $.PercServiceUtils.STATUS_SUCCESS){
                vardata = {url : site, totalLikes : results.data};
     ...
     * @param callback (function) function to call with response from server
     */
    vargetTotalLikesForThisPage = function(callback) {
		varlikePathname = likeId(window.location.pathname);
     ...
     * @param callback (function) function to call with response from server
     */
    vargetTotalLikes = function(site, id, type, callback) {
	 ...
	 * @return isLiked boolean true if liked, boolean false if not liked.
	 */
	varisLiked = function(url) {
		if("undefined" === typeof (url) || null === url) {
            return false;
        }
		
		varlikedState = $.cookie(url);
		if (null === likedState) {
            return false;
        }
	...
	varlikeId = function(pathname){
        ...
        {   
            varindexValue = pathname.lastIndexOf('index');
            if(indexValue !== -1) {
	 ...
	 */
	varsetThisLiked = function(liked,siteName) {
