/*[ TraceResultCompareHtmlCgiParam.java ]**************************************
 *
...
public class TraceResultCompareHtmlCgiParam extends TraceResultCompare
{
   ...
   public String readFilteredLine(BufferedReader reader) throws java.io.IOException
   {
      ...
      // see if starting a cgi vars
      if (line.toLowerCase().indexOf("cgi variables:") > -1)
      {
         inVars = true;
      }
      // see if in a node
      ...
      {
         // read lines till we hit a blank and see if it should be skipped
         while (line != null)
         {
            ...
            /* see if still skipping a bad var - skip till we hit another
             * line with an "=" in it
             */
            if (inNode)
            {
            ...
            {
               // see if line should be skipped
               if ((line.toUpperCase().indexOf("SERVER_NAME =") > -1) ||
                  (line.toUpperCase().indexOf("REMOTE_ADDR =") > -1))
               {
                  // these are lines to skip
                  inNode = true;
               }
               else
               {
   ...
   public static void main(String[] args)
   {
      ...
      try
      {
         ...
         // create the object
         TraceResultCompareHtmlCgiParam compareResult = new TraceResultCompareHtmlCgiParam();
   ...
   /**
    * tracks if currently reading lines within a CGI Vars block
    */
   private boolean inVars = false;

   /**
    * tracks if currently reading lines within "var = " line
    */
