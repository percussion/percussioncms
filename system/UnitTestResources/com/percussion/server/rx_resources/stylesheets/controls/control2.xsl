...
<xsl:template match="/" />
	<psxctl:ControlMeta name="control2" dimension="single" choiceset="none">
	...
	<xsl:template match="Control[@name='control2']" mode="psxcontrol">
		<input type="text" name="{@paramName}" value="{Value}">
			...
			<xsl:call-template name="parametersToAttributes">
				<xsl:with-param name="controlClassName" select="'control2'"/>
