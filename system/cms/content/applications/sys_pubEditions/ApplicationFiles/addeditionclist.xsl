...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="psxi18n">
	...
	<xsl:include href="file:sys_pubEditions/addeditionclist_body.xsl"/>
		...
		<html>
			<head>
				...
				<script type="text/javaScript" src="../sys_resources/js/formValidation.js"><![CDATA[
                                ...
                                <script type="text/javaScript" src="../sys_resources/js/href.js"><![CDATA[
          ]]></script> 
				<script type="text/javascript" id="clientEventHandlersJS" language="javascript"><![CDATA[
         function save_onclick() {
				if(document.addclist.contentlist.selectedIndex==0){
					alert("Content List: field is a required field");
					return false;
				}
				if(!(numberField(document.addclist.sequence.value,"Sequence"))){
					return false;
				}
				if(document.addclist.authtype.selectedIndex==0){
					alert("Authorization Type: field is a required field");
					return false;
				}
				if(document.addclist.context.selectedIndex==0){
					alert("Context: field is a required field");
					return false;
				}

				document.addclist.submit();
         }
         function cancelFunc() {
              var params = PSHref2Hash(window.location.href);
              document.location.href=document.addclist.doccancelurl.value + 
                 "?sys_componentname=" + 
                 document.addclist.sys_componentname.value +
                 "&sys_pagename=" +
                 params["sys_pagename"] +
                 "&editionid=" +
                 document.addclist.editionid.value;
         }
				...
				<table width="100%" cellpadding="0" cellspacing="1" border="0">
					<tr>
						...
						<td align="middle" width="100%" valign="top" height="100%" class="outerboxcell">
							...
							<xsl:apply-templates mode="addeditionclist_mainbody">
              </xsl:apply-templates>
							<!--   end main body slot   -->
						</td>
					</tr>
