...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="psxi18n">
   ...
   <xsl:include href="file:sys_pubEditions/copyedition_body.xsl"/>
         ...
         <head>
            ...
            <script type="text/javascript" language="javascript"><![CDATA[
               function create_onclick()
               {
                  ...
                  if (!(reqField(document.copyedition.neweditionname.value, "New Edition Name")))
                     return false;
               
                  // add hidden and user input html parameters to the redirect url
                  var bounceTo = document.copyedition.sourceeditionlist[document.copyedition.sourceeditionlist.selectedIndex].value;
                  var params = PSHref2Hash(bounceTo);
                  params["DBActionType"] = document.copyedition.DBActionType.value;
                  params["sys_componentname"] = document.copyedition.sys_componentname.value;
		  params["sys_pagename"] = document.copyedition.sys_pagename.value;
                  params["editiontitle"] = document.copyedition.neweditionname.value;
