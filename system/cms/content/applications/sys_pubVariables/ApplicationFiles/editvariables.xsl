...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="psxi18n">
	...
	<xsl:include href="file:sys_pubVariables/editvariables_body.xsl"/>
	...
	<xsl:template match="/">
		<html>
         ...
         function save_onclick() {
				if(!(reqField(document.editassemblerprop.assemblerpropname.value,"Name"))){
					return false;
				}				
				if(!(reqField(document.editassemblerprop.assemblerpropvalue.value,"Value"))){
					return false;
				}
				if(document.editassemblerprop.context[document.editassemblerprop.context.selectedIndex].value != 0 && document.editassemblerprop.sitelist[document.editassemblerprop.sitelist.selectedIndex].value == 0)
				{
					alert("Preview Site is allowed only for Preview Context. Please select a site.");
					return;
				}
				document.editassemblerprop.contextid.value = document.editassemblerprop.context[document.editassemblerprop.context.selectedIndex].value;
				document.editassemblerprop.siteid.value = document.editassemblerprop.sitelist[document.editassemblerprop.sitelist.selectedIndex].value;
				document.editassemblerprop.submit();
         }
         function cancelFunc() {
           document.location.href=document.editassemblerprop.doccancelurl.value;
         }
				...
				<table width="100%" cellpadding="0" cellspacing="1" border="0">
					<tr>
						...
						<td align="middle" width="100%" valign="top" height="100%" class="outerboxcell">
							...
							<xsl:apply-templates mode="editvariables_mainbody">
              </xsl:apply-templates>
							<!--   end main body slot   -->
						</td>
					</tr>
