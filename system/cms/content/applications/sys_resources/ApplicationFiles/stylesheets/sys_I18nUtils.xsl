...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml"
   ...
   <xsl:template name="getLocaleString">
      ...
      <xsl:value-of select="psxi18n:PSI18nUtils.getString($key, $lang)"/>
   </xsl:template>
   <!-- Template to return localized string for a given node -->
   <xsl:template match="*" mode="psxi18n">
      <xsl:param name="lang" select="''"/>
      <xsl:value-of select="psxi18n:PSI18nUtils.getString(., $lang)"/>
   </xsl:template>
   <xsl:template name="getFormatString">
      <xsl:param name="formatstring" select="''"/>
      <xsl:param name="params" select="''"/>
      <xsl:param name="lang" select="''"/>
      <xsl:value-of select="psxi18n:PSI18nUtils.formatMessage($formatstring, $params, $lang)"/>
   </xsl:template>
   ...
   <xsl:template name="getMnemonicLocaleString">
      ...
      <xsl:value-of disable-output-escaping="yes"
                    select="psxi18n:PSI18nUtils.getMnemonicString($key, $mnemonickey, $lang)"/>
   </xsl:template>
   <xsl:template name="getMnemonic">
      <xsl:param name="key" select="''"/>
      <xsl:param name="lang" select="''"/>
      <xsl:value-of select="psxi18n:PSI18nUtils.getMnemonic($key, $lang)"/>
   </xsl:template>
   <xsl:template name="getTooltip">
      <xsl:param name="key" select="''"/>
      <xsl:param name="lang" select="''"/>
      <xsl:value-of select="psxi18n:PSI18nUtils.getTooltip($key, $lang)"/>
   </xsl:template>
</xsl:stylesheet>
