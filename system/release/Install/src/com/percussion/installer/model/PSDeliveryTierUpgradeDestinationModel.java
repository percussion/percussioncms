 ...
 * Rhythmyx directory.
 */
public class PSDeliveryTierUpgradeDestinationModel extends RxIAModel
{
   /**
    ...
    * runtime platform.
    */
   public PSDeliveryTierUpgradeDestinationModel(IPSProxyLocator locator)
   {
      super(locator);
   ...
   public boolean queryExit()
   {
      ...
      if (getDestDir().equals(RxFileManager.OTHER_DIR))
      {
         ...
         PSDeliveryTierInstallOptionModel.upgradeInstall(true);
      }
      else
      {
         ...
         if (canContinue)
         {
            PSDeliveryTierInstallOptionModel.upgradeInstall(false);
            updateProductTree(getDestDir());
         }else{
            getUserInput("Validation Error", "Unable to validate the destination for upgrade.  Please check the installation location and try again.", null);
         }
      }
   ...
   {
      super.queryEnter();
      return m_isBrander ? true : PSDeliveryTierInstallOptionModel.checkUpgradeInstall();
   }
         ...
         try
         {
            if (isDirectoryWritable(dirName))
            {
               ...
               else
               {
                  ...
                  {
                     String errorText = "A DTS Server is running in the " +
   ...
   {   
      Set<?> upgradeDirs = PSDeliveryTierUpgradeFlag.getUpgradeDirectories();
