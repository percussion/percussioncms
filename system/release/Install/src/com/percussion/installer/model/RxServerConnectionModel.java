 ...
 */
public class RxServerConnectionModel extends RxIAModel
{  
   /**
    * Constructs an {@link RxServerConnectionModel} object.
    *  
    * @param locator the proxy locator which will retrieve the proxy used to
    * interact with the InstallAnywhere runtime platform.
    */
   public RxServerConnectionModel(IPSProxyLocator locator)
   {
      super(locator);
      setPersistProperties(new String[]{
            RxProtocolModel.DB_SERVER_NAME,
            RxProtocolModel.USER_ID,
            RxProtocolModel.PWD
      });
   ...
   public boolean queryExit()
   {
      ...
      try
      {
         conn = InstallUtil.createConnection(RxProtocolModel.fetchDriver(), dbServer, user, pwd);
         if (conn != null)
            bConnection = true;
      }
      finally
   ...
   private boolean isSupportedOracleVersion()
   {
      ...
      {
         RxLogger.logInfo(
               "RxServerConnectionModel#checkOracleVersion() : "
               + e.getMessage());
         RxLogger.logInfo(e);
      }
      finally
   ...
   {
      ms_dbServer = server;
      setValue(RxProtocolModel.DB_SERVER_NAME, server);
   ...
   {
      ms_user = user;
      setValue(RxProtocolModel.USER_ID, user);
