...
#****************************************************************************
#
# This script installs Rhythmyx Publisher as a daemon.
#
#****************************************************************************
...
#
# Updates the S15RhythmyxPublisherD shell ( setting the installation dir) and moves
# it to /etc/rcd.3
#
updateS15RhythmyxPublisherD()
{
   ...
   nawk -v loc=$loc -f tmp.awk S15RhythmyxPublisherD > tmp.sh
...
if [ ! -d $loc/bin ] ; then 
   echo "This script must be executed from the Rhythmyx Publisher root directory."
   echo "Aborting!"
   exit 1
fi
