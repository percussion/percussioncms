...
public abstract class PSDeployJob extends PSJobRunner implements IPSJobHandle
{
    ...
    * objects to use to determine the count, may not be <code>null</code> or
    * empty, each package must contain at least one included child dependency.
    * 
    * @throws IllegalArgumentException if <code>pkgs</code> is not valid.
    */
    ...
    * @param pkgs An iterator over one or more <code>PSDependency</code>
    * objects to use to determine the count, may not be <code>null</code> or
    * empty, each package must contain at least one counted child dependency.
    * @param includedOnly If <code>true</code>, only included dependencies will
    * be counted, otherwise all depedencies will be counted.
    * 
    * @throws IllegalArgumentException if <code>pkgs</code> is 
    * <code>null</code> or a package does not contain counted children.
    */
