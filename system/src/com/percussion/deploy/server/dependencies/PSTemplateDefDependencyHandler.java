        ...
        IPSIdTypeHandler
{
            ...
            throws PSDeployException
    {
        ...
        while (tmpNames.hasNext())
        {
            ...
            dep = createDeployableElement(m_def, String.valueOf(tmp.getGUID().longValue()), name);
            if ( dep != null )
                deps.add(dep);
        }
            ...
            throws PSDeployException
    {
        ...
        if (clMapping != null && clMapping.getTargetId() != null)
        {
            PSDependencyUtils.transformElementId(t,
                    dep, ctx, clMapping);
        }
                                       ...
                                       PSJexlBinding orig_b, PSJexlBinding new_b)
    {
        ...
        if ( StringUtils.isNotBlank(new_b.getName()))
        {
            b = t.getBindings().stream().filter(tmp_b -> tmp_b != null && tmp_b.getVariable() != null &&
                    tmp_b.getVariable().compareTo(new_b.getName()) == 0).findFirst().orElse(null);

        }
        else
        {
            b = t.getBindings().stream().filter(tmp_b -> tmp_b != null && tmp_b.getExpression() != null &&
                    tmp_b.getExpression().compareTo(orig_b.getExpression()) == 0).findFirst().orElse(null);
        }
        if ( b != null ) {
