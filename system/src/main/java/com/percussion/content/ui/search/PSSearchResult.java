...
public class PSSearchResult
{
   ...
   public String getSearchResults(IPSRequestContext request)
      ...
      if (search.useExternalSearch())
      {
         ...
         else
         {
            ...
            search.removeProperty(PSCommonSearchUtils.PROP_SYNONYM_EXPANSION,
                  null);
      ...
      // Get Slot ContentType Template Map
      
      String slotId = getParameter("slotId", null, parameters);
      boolean needsTemplate = isNotBlank(slotId) ? true : false;
      Map<IPSGuid, Set<IPSGuid>> slotContentTypeTemplateMap = getSlotContentTypeTemplateMap(slotId);
      ...
      for (IPSSearchResultRow row : list)
      {
         ...
         if (needsTemplate) {
            boolean hasTemplateForSlot = hasTemplateForSlot(
                  slotContentTypeTemplateMap, ctypeId);
            
            if (! hasTemplateForSlot )
               continue;
         }
   ...
   private boolean hasTemplateForSlot(
         Map<IPSGuid, Set<IPSGuid>> slotContentTypeTemplateMap, String ctypeId)
   {
      if (isBlank(ctypeId))
         return false;
      boolean hasTemplateForSlot = false;
      IPSGuid contentTypeGuid = new PSGuid(PSTypeEnum.NODEDEF, Integer.parseInt(ctypeId)); 
      Set<IPSGuid> templateIds = slotContentTypeTemplateMap.get(contentTypeGuid);
      if (templateIds != null && ! templateIds.isEmpty()) 
      {
         hasTemplateForSlot = true;
      }
      return hasTemplateForSlot;
   }

   private Map<IPSGuid, Set<IPSGuid>> getSlotContentTypeTemplateMap(
         String slotId)
   {
      Map<IPSGuid, Set<IPSGuid>> slotContentTypeTemplateMap = null;
      
      if (isNotBlank(slotId)) {
         IPSTemplateSlot slot = PSAssemblyServiceLocator.getAssemblyService()
            .findSlot(new PSGuid(PSTypeEnum.SLOT, Integer.parseInt(slotId)));
         if (slot != null)
            slotContentTypeTemplateMap = PSAssemblyServiceUtils.getSlotAssociationMap(slot);
      }
      return slotContentTypeTemplateMap;
   }
