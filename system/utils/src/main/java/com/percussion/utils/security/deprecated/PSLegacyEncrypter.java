...
import com.percussion.utils.security.PSAbstractEncryptor;
...
public class PSLegacyEncrypter extends PSAbstractEncryptor {
   ...
   @Deprecated
   public static String OLD_SECURITY_KEY(){
      try {
         return PSEncryptor.getInstance().decryptLegacyKey(
   ...
   @Deprecated
   public static String OLD_SECURITY_KEY2(){
      try {
         return PSEncryptor.getInstance().decryptLegacyKey(
   ...
   private static volatile PSLegacyEncrypter instance;
   ...
   public static PSLegacyEncrypter getInstance(){
      synchronized (PSLegacyEncrypter.class) {
         if (instance == null) {
            instance = new PSLegacyEncrypter();
         }
         return instance;
      }
   }
    ...
    */
   @Deprecated
   public static final String PART_TWO(){
      try {
         return PSEncryptor.getInstance().decryptLegacyKey(
    ...
    * decryption in {@link #decrypt(String, String)} must be a multiple of this
   ...
   public String encryptCredentials(String uid, String pw) {
      ...
      try {
         IPSKey key = PSEncryptionKeyFactory.getKeyGenerator("DES");

         byte[] encrData = pw.getBytes(PSCharSets.rxJavaEnc());
         ...
         try {
            ...
            com.percussion.util.PSBase64Encoder.encode(
                    new ByteArrayInputStream(encrData), bOut2);
            return bOut2.toString(PSCharSets.rxJavaEnc());
         } catch (Exception e) {
            if (bOut2 != null)
               bOut2.close();
            return null;
   ...
   public static final String CRYPT_KEY() {
      try {
         return PSEncryptor.getInstance().decryptLegacyKey("RapTr6hOh1KUXM5I0gvXYnaQKJDcO9AqlswePsbjnCHGQLk3H9ubfJU4VUIIvg==");
      } catch (PSEncryptionException e) {
         return "";
      }
   }
   ...
   @Deprecated
   @ToDoVulnerability
   public static final String EMAIL_CRYPT_KEY(){
      try {
         return PSEncryptor.getInstance().decryptLegacyKey("zDLBBy28QpbQ1Mjy2J+9MCJ/tFDkUydWTM9EuPM+MpXAWO1sKQR5");
      } catch (PSEncryptionException e) {
         return "";
      }
   }
   ...
   @Deprecated
   public final static String INVALID_DRIVER() {
      try {
         return PSEncryptor.getInstance().decryptLegacyKey(
   ...
   @Deprecated
   public final static String INVALID_CRED() {
      try {
         return PSEncryptor.getInstance().decryptLegacyKey(
